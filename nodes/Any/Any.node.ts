...
export class Any implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Any',
		name: 'any',
		icon: 'file:question_mark.svg',
		group: ['route'],
		version: 1,
		description:
		...
		outputNames: ['any', 'none', 'statistics'],
		properties: [
			{
			...
			},
			{
				...
				options: [
					{
						name: 'ALL',
						description: 'In case of any match, return the full list',
						value: 'all',
					},
					{
						name: 'MATCHES',
						description: 'In case of any match, return all matches',
						value: 'matches',
					},
					{
						name: 'FIRST',
						description: 'In case of any match, return the first match',
						value: 'first',
					},
				],
				default: 'matches',
				description: 'If any match is recognized, shall all data be returned or just matches?',
			},
		],
	};
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const returnStatistics: INodeExecutionData[] = [];
		...
		let statistics = {
			any: false,
			none: false,
			kept: returnDataTrue.length,
			dropped: returnDataFalse.length,
		};

		if (returnDataTrue.length == 0) {
			statistics.any = true;
			returnStatistics.push({ json: statistics });
			return [[], items, returnStatistics];
		} else {
			statistics.none = true;
			returnStatistics.push({ json: statistics });
			if (this.getNodeParameter('returnType', 0)?.toString() == 'matches') {
				return [returnDataTrue, [], returnStatistics];
			} else if (this.getNodeParameter('returnType', 0)?.toString() == 'first') {
				return [[returnDataTrue[0]], [], returnStatistics];
			} else {
				return [items, [], returnStatistics];
			}
		}
