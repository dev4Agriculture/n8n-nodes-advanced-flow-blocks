...
export class FilterAdvanced implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		if (returnDataTrue.length == 0) {
			statistics.dropAll = true;
		} else if (returnDataFalse.length == 0) {
			statistics.keepAll = true;
		}
		returnStatistics.push({ json: statistics });

		return [returnDataTrue, returnDataFalse, returnStatistics];
	}
}
